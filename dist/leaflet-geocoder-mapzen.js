...
var Geocoder = L.Control.extend({
  ...
  options: {
    position: 'topleft',
    attribution: 'Geocoding by dx-tech',
  ...
  initialize: function (apiKey, options) {
    ...
    if (window.XDomainRequest) {
      ...
      this.options.url = 'https://192.168.99.67:3101/v1';
    }
    ...
    if (options && typeof options.latlng !== 'undefined') {
      ...
      console.warn('[leaflet-geocoder] DEPRECATION WARNING:',
  ...
  reset: function () {
    ...
    var objs = document.getElementsByClassName("leaflet-pelias-label");
    var width = this._container.clientWidth - this._radio_width;
    for(var i = 0; i < objs.length; i ++){
        objs[i].style.marginLeft = (width).toString() + 'px';;
    }
    this.removeMarkers();
    this.clearResults();
    this.fire('reset');
  ...
  getLayers: function (params) {
    ...
    if (this.options.select) {
        var checked = [];
        var objs = document.getElementsByName("layer");
        for(var i = 0; i < objs.length; i ++){
            if(objs[i].checked)
                checked.push(objs[i].value);
        }
        if (checked.length > 0)
            layers = checked;
    }
  ...
  expand: function () {
    ...
    this._container_width = this._container_init_width;
    this.setFullWidth();
    this.fire('expand');
  },
  ...
  collapse: function () {
    ...
    this._container_width = this._search.clientWidth;
    this._input.blur();
    this.clearFullWidth();
    this.clearResults();
    this.fire('collapse');
    ...
    if (this.options.fullWidth) {
      ...
      this._container.style.width = width.toString() + 'px';
        ...
        if (this._input.value.length > 0)
            reset_width = this._reset_width;
        var objs = document.getElementsByClassName("leaflet-pelias-label");
        for(var i = 0; i < objs.length; i ++){
            objs[i].style.display = "block";
            objs[i].style.marginLeft = (width - this._radio_width - reset_width).toString() + 'px';;
        }
    }
  },
  ...
  clearFullWidth: function () {
    ...
    if (this.options.fullWidth) {
        ...
        if (this._container_width === undefined) {
            this._container_init_width = this._container.clientWidth;
            this._container_width = this._container_init_width;
        }
        this._search_width = this._search.clientWidth;
        var reset_width = 0;
        if (this._input.value.length > 0)
            reset_width = this._reset_width;
        var objs = document.getElementsByClassName("leaflet-pelias-label");
        for(var i = 0; i < objs.length; i ++) {
            if (this._container_width - this._radio_width <= this._search_width)
                objs[i].style.display = "none";
            else {
                objs[i].style.display = "block";
                objs[i].style.marginLeft = (this._container_width - this._radio_width - reset_width).toString() + 'px';;
            }
        }
    }
  },
  ...
  onAdd: function (map) {
    ...
    if (this.options.select) {
        this._container_width = this._container.clientWidth;
        var label = L.DomUtil.create('label', 'leaflet-pelias-label', this._container);
        label.style = "position: absolute;margin-left:"+this._container_width.toString()+"px";
        this._radio_poi = L.DomUtil.create('input', 'leaflet-pelias-radio', label);
        this._radio_poi.type = "radio";
        this._radio_poi.name = "layer";
        this._radio_poi.checked = "true";
        var textnode = document.createTextNode("poi");
        label.appendChild(textnode);
        this._radio_poi.value = "pois";
        
        L.DomEvent.addListener(this._radio_poi, 'click', function (e) {
            if (this._input.value.length > 0)
                this.autocomplete(this._input.value);
        }, this);
        
        label = L.DomUtil.create('label', 'leaflet-pelias-label', this._container);
        label.style = "position: absolute;margin-top: 20px;margin-left:"+this._container_width.toString()+"px";
        this._radio_address = L.DomUtil.create('input', 'leaflet-pelias-radio', label);
        this._radio_address.type = "radio";
        this._radio_address.name = "layer";
        textnode = document.createTextNode("addr");
        label.appendChild(textnode);
        this._radio_address.value = "address";
        
        L.DomEvent.addListener(this._radio_address, 'click', function (e) {
            if (this._input.value.length > 0)
                this.autocomplete(this._input.value);
        }, this);
    }
    ...
    this._reset_width = this._reset.clientWidth;
      ...
      .on(this._input, 'keyup', function (e) {
        ...
        if (text.length > 0) {
          L.DomUtil.removeClass(this._reset, 'leaflet-pelias-hidden');
            ...
            var width = this._container.clientWidth - this._radio_width;
            for(var i = 0; i < objs.length; i ++){
                objs[i].style.marginLeft = (width - this._reset_width).toString() + 'px';;
            }
        } else {
            this._reset_width = this._reset.clientWidth;
          L.DomUtil.addClass(this._reset, 'leaflet-pelias-hidden');
